    ...
    switch (source) {
        ...
        case ObjectSource::LOCAL_VARIABLE:
            ...
            return (dest == ObjectDestination::RETURN || 
                   dest == ObjectDestination::STORAGE);
        ...
        case ObjectSource::ATTRIBUTE_ACCESS:
            // 索引和属性访问在修改时需要复制
            return (dest == ObjectDestination::ASSIGNMENT || 
                   dest == ObjectDestination::RETURN ||
                   dest == ObjectDestination::STORAGE);
...
bool ObjectLifecycleManager::needsIncRef(ObjectType* type, ObjectSource source, ObjectDestination dest) {
    ...
    switch (dest) {
        ...
        case ObjectDestination::RETURN:
            ...
            return (source == ObjectSource::LOCAL_VARIABLE || 
                   source == ObjectSource::PARAMETER ||
                   source == ObjectSource::INDEX_ACCESS ||
                   source == ObjectSource::ATTRIBUTE_ACCESS);
...
bool ObjectLifecycleManager::needsDecRef(ObjectType* type, ObjectSource source) {
    ...
    // 临时对象需要在使用后减少引用计数
    return (source == ObjectSource::BINARY_OP || 
           source == ObjectSource::UNARY_OP || 
           source == ObjectSource::FUNCTION_RETURN);
}
...
bool ObjectLifecycleManager::needsWrapping(llvm::Value* value, ObjectType* type) {
    ...
    bool isPrimitiveValue = value->getType()->isIntegerTy() || 
...
bool ObjectLifecycleManager::areTypesCompatible(ObjectType* typeA, ObjectType* typeB) {
    ...
    if (isASequence && isBSequence) {
        ...
        if ((typeA->getTypeId() == PY_TYPE_STRING && dynamic_cast<const ListType*>(typeB)) ||
            (typeB->getTypeId() == PY_TYPE_STRING && dynamic_cast<const ListType*>(typeA))) {
            return false;
        }
        return true;
    }
...
//===----------------------------------------------------------------------===//
// 确定表达式对象的来源
ObjectLifecycleManager::ObjectSource CodeGenLifecycleManager::determineObjectSource(ExprAST* expr) {
...
llvm::Value* ObjectLifecycleManager::createObject(PyCodeGen& gen, llvm::Value* value, int typeId) {
    ...
    switch (typeId) {
        ...
        case PY_TYPE_DOUBLE: {
            ...
            llvm::Function* createDoubleFunc = llvm::Function::Create(
                funcType, 
                llvm::Function::ExternalLinkage,
                "py_create_double", 
                gen.getModule()
        ...
        case PY_TYPE_BOOL: {
            ...
            llvm::Function* createBoolFunc = llvm::Function::Create(
                funcType, 
                llvm::Function::ExternalLinkage,
                "py_create_bool", 
                gen.getModule()
...
llvm::Value* ObjectLifecycleManager::adjustObject(
    ...
    if (type->isReference() && needsIncRef(type, source, dest)) {
        ...
        llvm::Function* incRefFunc = llvm::Function::Create(
            incRefFuncType, 
            llvm::Function::ExternalLinkage,
            "py_incref", 
            gen.getModule()
...
llvm::Value* ObjectFactory::createList(PyCodeGen& gen, llvm::Value* size, ObjectType* elemType) {
    ...
    llvm::Function* createListFunc = llvm::Function::Create(
        createListFuncType, 
        llvm::Function::ExternalLinkage,
        "py_create_list", 
        gen.getModule()
...
bool ObjectTypeChecker::isOfType(PyCodeGen& gen, llvm::Value* value, ObjectType* type) {
    ...
    llvm::Function* checkTypeFunc = llvm::Function::Create(
        checkTypeFuncType, 
        llvm::Function::ExternalLinkage,
        "py_check_type", 
        gen.getModule()
