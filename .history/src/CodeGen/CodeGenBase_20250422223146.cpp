...
void PySymbolTable::dump(std::ostream& out) const
{
    ...
    // 创建一个临时向量来保存指向作用域的指针，以便迭代
    // 而不修改原始堆栈。
    std::vector<const PyScope*> scopePtrs;
    std::stack<std::unique_ptr<PyScope>> tempStack = scopes; // 复制堆栈

    while (!tempStack.empty()) {
        scopePtrs.push_back(tempStack.top().get());
        tempStack.pop();
    }
    // scopePtrs 现在从栈顶（当前作用域）到底部（全局作用域）保存作用域

    // 从顶部（当前）作用域向下迭代到全局作用域
    for (size_t i = 0; i < scopePtrs.size(); ++i) {
        const PyScope* scope = scopePtrs[i];
        // 作用域深度：size() 是总深度，size() - i 是从 1 开始的深度（1=全局）
        size_t depth = scopePtrs.size() - i;
        out << "--- Scope Level " << depth << " ---\n";

        // 打印此作用域中直接定义的变量和类型
        // 注意：getVariable/getVariableType 会向上查找，这里我们只显示当前层级的内容
        // 需要访问 PyScope 的内部 map，假设 PyScope 提供了访问器
        // （如果 PyScope 没有提供，需要添加 const 访问器）
        // 假设 PyScope 有 getVariables() 和 getVariableTypes() const 方法
        out << "  Variables & Types (Defined in this scope):\n";
        const auto& vars = scope->getVariables(); // 假设返回 const std::map<std::string, llvm::Value*>&
        const auto& types = scope->getVariableTypes(); // 假设返回 const std::map<std::string, ObjectType*>&
        ...
        if (vars.empty() && types.empty()) {
         ...
         // 打印此作用域中定义的函数 AST
         // 假设 PyScope 有 getFunctionDefinitions() const 方法
        out << "  Function ASTs (Defined in this scope):\n";
        const auto& funcDefs = scope->getFunctionDefinitions(); // 假设返回 const std::unordered_map<std::string, const FunctionAST*>&
        if (funcDefs.empty()) {
            out << "    (None)\n";
        } else {
