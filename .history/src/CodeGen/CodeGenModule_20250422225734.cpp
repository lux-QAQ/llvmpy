...
bool CodeGenModule::generateModule(ModuleAST* module, bool isEntryPoint)
{
    ...
    if (isEntryPoint)
    {
        ...
        if (entryFn->empty())
        {
            ...
            if (!entryBB) { // Check if block creation failed
        ...
        else if (!lastBlock || lastBlock->getParent() != entryFn)
        {
            ...
            if (lastBlock && !lastBlock->getTerminator()) { // Check if we can add a return
    ...
    else // Non-entry point module
    {
        ...
        for (const auto& stmt : module->getStatements())
        {
            ...
            if (stmt->kind() != ASTKind::FunctionDefStmt &&
                stmt->kind() != ASTKind::ClassStmt && // Assuming ClassStmt is definition-like
...
llvm::Value* CodeGenModule::handleFunctionDef(const FunctionAST* funcAST) // 接受 const*, 返回 Value*
{
    ...
    if (!function->empty()) {
        ...
        if (savedIP.getBlock()) codeGen.getBuilder().restoreIP(savedIP);
#ifdef DEBUG_CODEGEN_handleFunctionDef
        DEBUG_LOG_DETAIL("HdlFuncDef", "Function already defined. Restoring context and returning existing function's object (if possible - needs lookup).");
        // TODO: Need a way to get the existing PyObject* if we skip redefinition.
        // For now, returning nullptr might be safer, or log an error.
#endif
        // Returning nullptr because we don't have the PyObject* readily available here.
        // The caller (handleFunctionDefStmt) should handle this potentially null return.
        return nullptr; // Or find a way to return the existing PyObject*
    }
    ...
    // --- 获取 Python 函数类型 (ObjectType*) ---
#ifdef DEBUG_CODEGEN_handleFunctionDef
    DEBUG_LOG_DETAIL("HdlFuncDef", "Calling TypeGen->getFunctionObjectType for '" + funcName + "'...");
#endif
    ...
    {
#ifdef DEBUG_CODEGEN_handleFunctionDef
        DEBUG_LOG_DETAIL("HdlFuncDef", "TypeGen->getFunctionObjectType FAILED for '" + funcName + "'.");
#endif
        codeGen.logError("Failed to get valid FunctionType for function: " + funcName, funcAST->line.value_or(0), funcAST->column.value_or(0));
...
#ifdef DEBUG_CODEGEN_handleFunctionDef
    DEBUG_LOG_DETAIL("HdlFuncDef", "TypeGen->getFunctionObjectType SUCCEEDED for '" + funcName + "'. Type: " + funcObjectType->getName());
#endif

    // --- 创建 Python 函数对象 (PyObject*) ---
#ifdef DEBUG_CODEGEN_handleFunctionDef
    DEBUG_LOG_DETAIL("HdlFuncDef", "Calling RuntimeGen->createFunctionObject for '" + funcName + "'...");
#endif
    llvm::Value* pyFuncObj = codeGen.getRuntimeGen()->createFunctionObject(function, funcObjectType);
    if (!pyFuncObj)
    {
#ifdef DEBUG_CODEGEN_handleFunctionDef
        DEBUG_LOG_DETAIL("HdlFuncDef", "RuntimeGen->createFunctionObject FAILED for '" + funcName + "'.");
#endif
        codeGen.logError("Failed to create Python function object for: " + funcName, funcAST->line.value_or(0), funcAST->column.value_or(0));
        // Restore context and cleanup
        codeGen.setCurrentFunction(savedFunction);
        codeGen.setCurrentReturnType(savedReturnTypeCtx);
        if (savedIP.getBlock()) codeGen.getBuilder().restoreIP(savedIP);
...
#ifdef DEBUG_CODEGEN_handleFunctionDef
    DEBUG_LOG_DETAIL("HdlFuncDef", "RuntimeGen->createFunctionObject SUCCEEDED for '" + funcName + "'. Value: " + llvmObjToString(pyFuncObj));
#endif
    ...
    // --- **关键：在函数作用域内绑定函数对象，以便递归** ---
#ifdef DEBUG_CODEGEN_handleFunctionDef
    DEBUG_LOG_DETAIL("HdlFuncDef", "Binding '" + funcName + "' to PyObject* " + llvmObjToString(pyFuncObj) + " in current (function) scope.");
#endif
    // 直接存储 PyObject*，因为这是在函数内部。
    // 如果是顶层函数，外层 (handleFunctionDefStmt) 会用全局变量覆盖这个绑定。
    ...
    for (const auto& stmt : funcAST->getBody())
    {
        ...
        if (!currentBlock || currentBlock->getParent() != function) {
            codeGen.logError("Internal error: Builder lost context during function body generation for " + funcName);
            break; // Stop processing statements in this function
        }
        if (currentBlock->getTerminator())
        {
#ifdef DEBUG_CODEGEN_handleFunctionDef
            DEBUG_LOG_DETAIL("HdlFuncDef", "Body: Block '" + llvmObjToString(currentBlock) + "' terminated early. Skipping remaining statements.");
#endif
            break; // Stop if block is terminated (e.g., by return)
        }
#ifdef DEBUG_CODEGEN_handleFunctionDef
    ...
    if (lastBlock && lastBlock->getParent() == function && !lastBlock->getTerminator())
    {
        ...
        if (returnType->isVoid())
        {
            codeGen.getBuilder().CreateRetVoid();
#ifdef DEBUG_CODEGEN_handleFunctionDef
            DEBUG_LOG_DETAIL("HdlFuncDef", "Added RetVoid.");
#endif
        }
        else
        {
            llvm::Value* noneValue = codeGen.getExprGen()->createNoneLiteral(); // Use ExprGen helper
            codeGen.getBuilder().CreateRet(noneValue);
#ifdef DEBUG_CODEGEN_handleFunctionDef
            DEBUG_LOG_DETAIL("HdlFuncDef", "Added Ret None (" + llvmObjToString(noneValue) + ").");
#endif
        }
    }
#ifdef DEBUG_CODEGEN_handleFunctionDef
    ...
    if (savedIP.getBlock())
    {
        codeGen.getBuilder().restoreIP(savedIP);
#ifdef DEBUG_CODEGEN_handleFunctionDef
        DEBUG_LOG_DETAIL("HdlFuncDef", "Restored IP to block " + llvmObjToString(savedIP.getBlock()));
#endif
    }
    else
    {
#ifdef DEBUG_CODEGEN_handleFunctionDef
        DEBUG_LOG_DETAIL("HdlFuncDef", "Saved IP was null, not restoring IP.");
#endif
    }
#ifdef DEBUG_CODEGEN_handleFunctionDef
    ...
    return pyFuncObj; // <--- 返回创建的 Python 函数对象
}
